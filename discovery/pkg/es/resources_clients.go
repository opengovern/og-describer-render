...
import (
	"context"
	render "github.com/opengovern/og-describer-render/discovery/provider"
...
// ==========================  START: PostgresqlBackup =============================

type PostgresqlBackup struct {
	ResourceID      string                             `json:"resource_id"`
	PlatformID      string                             `json:"platform_id"`
	Description     render.PostgresqlBackupDescription `json:"Description"`
	Metadata        render.Metadata                    `json:"metadata"`
	DescribedBy     string                             `json:"described_by"`
	ResourceType    string                             `json:"resource_type"`
	IntegrationType string                             `json:"integration_type"`
...
type PostgresqlBackupHit struct {
	ID      string           `json:"_id"`
	Score   float64          `json:"_score"`
	Index   string           `json:"_index"`
	Type    string           `json:"_type"`
	Version int64            `json:"_version,omitempty"`
	Source  PostgresqlBackup `json:"_source"`
	Sort    []interface{}    `json:"sort"`
}

type PostgresqlBackupHits struct {
	Total essdk.SearchTotal     `json:"total"`
	Hits  []PostgresqlBackupHit `json:"hits"`
}

type PostgresqlBackupSearchResponse struct {
	PitID string               `json:"pit_id"`
	Hits  PostgresqlBackupHits `json:"hits"`
}

type PostgresqlBackupPaginator struct {
	paginator *essdk.BaseESPaginator
}

func (k Client) NewPostgresqlBackupPaginator(filters []essdk.BoolFilter, limit *int64) (PostgresqlBackupPaginator, error) {
	paginator, err := essdk.NewPaginator(k.ES(), "render_postgresql_backup", filters, limit)
	if err != nil {
		return PostgresqlBackupPaginator{}, err
	}

	p := PostgresqlBackupPaginator{
		paginator: paginator,
	}
...
func (p PostgresqlBackupPaginator) HasNext() bool {
	return !p.paginator.Done()
}

func (p PostgresqlBackupPaginator) Close(ctx context.Context) error {
	return p.paginator.Deallocate(ctx)
}

func (p PostgresqlBackupPaginator) NextPage(ctx context.Context) ([]PostgresqlBackup, error) {
	var response PostgresqlBackupSearchResponse
	err := p.paginator.Search(ctx, &response)
	if err != nil {
		return nil, err
	}

	var values []PostgresqlBackup
	for _, hit := range response.Hits.Hits {
		values = append(values, hit.Source)
	}
...
var listPostgresqlBackupFilters = map[string]string{
	"created_at": "Description.CreatedAt",
	"id":         "Description.ID",
	"url":        "Description.URL",
}

func ListPostgresqlBackup(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	plugin.Logger(ctx).Trace("ListPostgresqlBackup")
	runtime.GC()
	...
	ke, err := essdk.NewClientCached(cfg, d.ConnectionCache, ctx)
	if err != nil {
		plugin.Logger(ctx).Error("ListPostgresqlBackup NewClientCached", "error", err)
		return nil, err
	}
	k := Client{Client: ke}
	...
	sc, err := steampipesdk.NewSelfClientCached(ctx, d.ConnectionCache)
	if err != nil {
		plugin.Logger(ctx).Error("ListPostgresqlBackup NewSelfClientCached", "error", err)
		return nil, err
	}
	integrationId, err := sc.GetConfigTableValueOrNil(ctx, steampipesdk.OpenGovernanceConfigKeyIntegrationID)
	if err != nil {
		plugin.Logger(ctx).Error("ListPostgresqlBackup GetConfigTableValueOrNil for OpenGovernanceConfigKeyIntegrationID", "error", err)
		return nil, err
	}
	encodedResourceCollectionFilters, err := sc.GetConfigTableValueOrNil(ctx, steampipesdk.OpenGovernanceConfigKeyResourceCollectionFilters)
	if err != nil {
		plugin.Logger(ctx).Error("ListPostgresqlBackup GetConfigTableValueOrNil for OpenGovernanceConfigKeyResourceCollectionFilters", "error", err)
		return nil, err
	}
	clientType, err := sc.GetConfigTableValueOrNil(ctx, steampipesdk.OpenGovernanceConfigKeyClientType)
	if err != nil {
		plugin.Logger(ctx).Error("ListPostgresqlBackup GetConfigTableValueOrNil for OpenGovernanceConfigKeyClientType", "error", err)
		return nil, err
	}

	paginator, err := k.NewPostgresqlBackupPaginator(essdk.BuildFilter(ctx, d.QueryContext, listPostgresqlBackupFilters, integrationId, encodedResourceCollectionFilters, clientType), d.QueryContext.Limit)
	if err != nil {
		plugin.Logger(ctx).Error("ListPostgresqlBackup NewPostgresqlBackupPaginator", "error", err)
		return nil, err
	}
	...
	for paginator.HasNext() {
		page, err := paginator.NextPage(ctx)
		if err != nil {
			plugin.Logger(ctx).Error("ListPostgresqlBackup paginator.NextPage", "error", err)
			return nil, err
		}
...
var getPostgresqlBackupFilters = map[string]string{
	"created_at": "Description.CreatedAt",
	"id":         "Description.ID",
	"url":        "Description.URL",
}

func GetPostgresqlBackup(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	plugin.Logger(ctx).Trace("GetPostgresqlBackup")
	runtime.GC()
	// create service
	cfg := essdk.GetConfig(d.Connection)
	ke, err := essdk.NewClientCached(cfg, d.ConnectionCache, ctx)
	...
	limit := int64(1)
	paginator, err := k.NewPostgresqlBackupPaginator(essdk.BuildFilter(ctx, d.QueryContext, getPostgresqlBackupFilters, integrationId, encodedResourceCollectionFilters, clientType), &limit)
	if err != nil {
		return nil, err
	}
